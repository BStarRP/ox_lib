            ...
            for action, actions in pairs(listeners) do
                ...
                for i = 1, #actions do
                    ...
                    if listener.net then
                        TriggerClientEvent(actionEvent, ...)
                    end
                end
                eventLog(event, 'Event Listeners Triggered', action, #listeners)
            end
...
function lib.event.trigger(resource, action, cb, net)
    ...
    local route = net and 'NET' or 'LOCAL'
    ...
    if event then
        ...
        triggerEvent = getRoute(net)
        triggerEvent(eventName, ...)
    else
        eventLog('No event was found for action: '..action..' in resource: '..resource..' with route: '..route)
    end
end
